...
final class MonoidSpec extends WordSpecLike with Matchers {
  ...
  "Monoid" should {
    ...
    "verify instances" in {
      ...
      List(Set(1, 2, 3), Set(2, 3, 4)).foldLeft(setUnionMonoid[Int].empty)(setUnionMonoid[Int].combine) shouldBe Set(1, 2, 3, 4)

      val optionIntMonoid = optionMonoid(intAdditionMonoid)
      List(Some(1), Some(2), Some(3)).foldLeft(optionIntMonoid.empty)(optionIntMonoid.combine) shouldBe Some(6)
